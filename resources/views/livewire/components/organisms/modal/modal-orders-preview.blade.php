<x-organisms.modal max-width="700px">

    @section('modal_title')
        <div>
            <button wire:click.prevent="" class="ui button tiny">Send</button>
            <a href="/download-pdf/{{ $order['code'] }}" class="ui button tiny">Download PDF</a>
        </div>
        <div>
            
        </div>
    <a href="{{ url('preview-pdf/') }}">pdf</a>
    @endsection

    @section('modal_body')


        @include('livewire.components.organisms.pagecontents.preview-download-pdf')
