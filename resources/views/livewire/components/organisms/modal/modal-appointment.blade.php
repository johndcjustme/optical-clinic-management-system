<x-organisms.modal on-close="@wire.closeModal">
    ...
    @section('modal_title')
        {{-- <div class="full_w flex flex_x_between full_w"> --}}
            <div class="x-flex x-flex-ycenter x-gap-1" style="gap:0.7em">
                @if ($modal['settings'] || $modal['settings2'])
                    ...
                    <x-atoms.ui.button-circular wire:click.prevent="$toggle('modal.settings2')" class="mini basic">
                        <i class="icon {{ $modal['settings2'] ? 'arrow left' : 'settings'}}"></i>
                    </x-atoms.ui.button-circular>
                    {{-- <button  class="circular ui icon button mini basic">
                    </button> --}}
                @endif
                @if ($isUpdate)
        ...
        @if ($modal['settings'])
            ...
            @if ($modal['settings2'])
                <div class="ui action input fluid">
                    ...
                    <button class="ui button">Add Category</button>
                </div>
            @else
                            ...
                            <x-slot name="tbody">
                                @foreach (App\Models\Day::all() as $day)
                                    <tr wire:click.prevent="selectedDay({{$day->id}})">
                                        <td class="@if (!$day->status) error @endif" style="padding-left:1em;">
                                            @if (!$day->status) <i class="attention icon"></i> @endif
                                            {{ $day->day}}
                                        </td>
                                        <td class="@if (!$day->status) error @endif right aligned" style="width: 10em;">
